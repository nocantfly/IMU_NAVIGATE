...
// Bank 3
#define ICM20X_B3_I2C_MST_ODR_CONFIG 0x00 ///< Sets ODR for I2C master bus
#define ICM20X_B3_I2C_MST_CTRL 0x01       ///< I2C master bus config
#define ICM20X_B3_I2C_MST_DELAY_CTRL 0x02 ///< I2C master bus config
#define ICM20X_B3_I2C_SLV0_ADDR                                                \
  0x03 ///< Sets I2C address for I2C master bus slave 0
#define ICM20X_B3_I2C_SLV0_REG                                                 \
  0x4 ///< Sets register address for I2C master bus slave 0
#define ICM20X_B3_I2C_SLV0_CTRL 0x05 ///< Controls for I2C master bus slave 0
#define ICM20X_B3_I2C_SLV0_DO 0x06   ///< Sets I2C master bus slave 0 data out
...
public:
	Adafruit_ICM20X();
	~Adafruit_ICM20X();
	void setSPI_Pins(SPI_HandleTypeDef *hspi, GPIO_TypeDef *cs_port, uint16_t cs_pin) {
	        spi_han = hspi;
	        _cs_port = cs_port;
	        _cs_pin = cs_pin;
	    }
...
//			int8_t mosi_pin, int32_t sensor_id = 0);
    ...
    bool readRegister_Public(uint16_t mem_addr, uint8_t *dest, uint16_t size) {
        return readRegister(mem_addr, dest, size);
    }
	...
	bool writeMagRegisterSLV0(uint8_t reg, uint8_t value);
	bool writeMagRegisterSLV4(uint8_t reg, uint8_t value);
	...
	virtual bool setupMag(void) { return true; }
protected:
